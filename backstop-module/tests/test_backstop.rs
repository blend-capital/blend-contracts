...
fn test_draw_happy_path() {
    ...
    e.as_contract(&pool_2, || {
        backstop_client.draw(&pool_2, &100_000_0000000, &pool_2_id);
    });
...
#[test]
fn test_draw_fails_not_authorized() {
    let e = Env::default();
    ...
    // draw
    let result = backstop_client.with_source_account(&sauron).try_draw(
        &pool_2,
        &100_000_0000000,
        &pool_2_id,
    );
...
#[test]
fn test_draw_fails_insufficient() {
    let e = Env::default();
    ...
    e.as_contract(&pool_2, || {
        ...
        match result {
            ...
            Err(error) => match error {
                Ok(p_error) => assert_eq!(p_error, BackstopError::InsufficientFunds),
                Err(s_error) => assert_eq!(s_error, Status::from_contract_error(6)),
            },
        }
    });
}
