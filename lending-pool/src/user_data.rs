...
use soroban_sdk::{BigInt, Env, BytesN};

use crate::{storage::{PoolDataStore, StorageManager}, user_config::{UserConfig, UserConfigurator}, dependencies::{OracleClient, TokenClient}};
...
impl UserData {
    pub fn load(e: &Env, user: &Identifier, action: &UserAction) -> UserData {
        ...
        UserData { e_collateral_base, e_liability_base }
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{storage::{ReserveConfig, ReserveData}, testutils::{create_token_contract, create_mock_oracle, generate_contract_id}};

    use super::*;
    use soroban_auth::Signature;
    use soroban_sdk::{BigInt, testutils::Accounts};
    ...
    fn test_to_effective_balance() {
        ...
        assert_eq!(to_effective_balance(&e, protocol_tokens, rate, ltv_factor, oracle_price), expected_e_balance);
    }
    ...
    fn test_load_user_only_collateral() {
        ...
        e.as_contract(&pool_id, || { storage.set_user_config(user_id.clone(), 0x0000000000000008) });
        ...
        e.as_contract(&pool_id, || {
            ...
            assert_eq!(user_data.e_collateral_base, BigInt::from_u64(&e, 38_5000000));
        });        
    }
    ...
    fn test_load_user_only_liability() {
        ...
        e.as_contract(&pool_id, || { storage.set_user_config(user_id.clone(), 0x0000000000000001) });
    ...
    fn test_load_user_only_action() {
        ...
        e.as_contract(&pool_id, || {
            let user_data = UserData::load(&e, &user_id, &user_action);
            assert_eq!(user_data.e_liability_base, BigInt::zero(&e));
            assert_eq!(user_data.e_collateral_base, BigInt::from_u64(&e, 22_5000000));
        });
    }
    ...
    fn test_load_user_all_positions() {
        ...
        e.as_contract(&pool_id, || { storage.set_user_config(user_id.clone(), 0x0000000000000006) }); // ...01_10
