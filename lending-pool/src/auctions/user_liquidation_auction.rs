...
use super::{get_fill_modifiers, AuctionType};
...
pub fn create_user_liq_auction_data(
    ...
    let position_data = PositionData::calculate_from_positions(e, &mut pool, &user_positions);
    ...
    for i in 0..reserve_list.len() {
        ...
        if b_token_balance > 0 {
            liquidation_quote
                .lot
                .set(res_asset_address.clone(), b_token_balance);
        }
        ...
        if d_token_balance > 0 {
            ...
            liquidation_quote
                .bid
                .set(res_asset_address, to_buy_amt_d_token);
        }
    }

    // ensure liquidation size is fair and the collateral is large enough to allow for the auction to price the liquidation
    let weighted_cf = position_data.collateral_base
        * 100
            .fixed_div_floor(position_data.collateral_raw * 100, oracle_scalar)
            .unwrap_optimized();
    ...
    if percent_liquidated == 1 {
        ...
        let new_hf = PositionData::calculate_from_positions(e, &mut pool, &user_positions)
            .as_health_factor();
...
pub fn fill_user_liq_auction(
    ...
    filler_positions.add_positions(
        e,
        &pool,
        auction_data.lot.clone(),
        auction_data.bid.clone(),
...
mod tests {
    ...
    fn test_create_user_liquidation_auction() {
        ...
        e.as_contract(&pool_address, || {
            ...
            assert_eq!(result.bid.get_unchecked(underlying_2), 0_7000000);
            assert_eq!(result.bid.len(), 1);
            assert_eq!(result.lot.get_unchecked(underlying_0), 20_0000000);
    ...
    fn test_fill_user_liquidation_auction() {
        ...
        let auction_data = AuctionData {
            bid: map![&e, (underlying_2.clone(), 0_7000000)],
            lot: map![&e, (underlying_0.clone(), 30_0000000)],
            block: 176,
        };
        let liq_pct = 0_4363636;
        let pool_config = PoolConfig {
        ...
        e.as_contract(&pool_address, || {
            storage::set_user_positions(&e, &samwise, &positions);
            storage::set_pool_config(&e, &pool_config);
            let result = create_user_liq_auction_data(&e, &samwise, liq_pct.clone());

            assert_eq!(result.block, 176);
            assert_eq!(result.bid.get_unchecked(underlying_2), 0_7000000);
            assert_eq!(result.bid.len(), 1);
            assert_eq!(result.lot.get_unchecked(underlying_0), 30_0000000);
            assert_eq!(result.lot.len(), 1);
            //scale up modifiers
            e.ledger().set(LedgerInfo {
            ...
            e.budget().reset_unlimited();
            fill_user_liq_auction(&e, &auction_data, &samwise, &frodo);
    ...
    fn test_create_fill_user_liquidation_auction_hits_target() {
        ...
        e.as_contract(&pool_address, || {
            ...
            assert_eq!(result.block, 51);
            assert_eq!(
                result.bid.get_unchecked(underlying_1.clone()),
                200_7500000_0000000
            );
            assert_eq!(result.bid.len(), 1);
            assert_eq!(result.lot.get_unchecked(underlying_0.clone()), 3000_0000000);
            assert_eq!(result.lot.len(), 1);
            ...
            //liquidate user
            let auction_data = AuctionData {
                bid: map![&e, (underlying_1, 200_7500000_0000000)],
                lot: map![&e, (underlying_0, 3000_0000000)],
                block: 50,
            };
            fill_user_liq_auction(&e, &auction_data, &samwise, &frodo);
    ...
    fn test_liquidate_user_dust_collateral() {
        ...
        e.as_contract(&pool_address, || {
            ...
            assert_eq!(result.block, 51);
            assert_eq!(result.bid.get_unchecked(underlying_1.clone()), 2_7500000);
            assert_eq!(result.bid.len(), 1);
            assert_eq!(result.lot.get_unchecked(underlying_0.clone()), 0_00000001);
            assert_eq!(result.lot.len(), 1);
            ...
            // liquidate user
            let auction_data = AuctionData {
                bid: map![&e, (underlying_1, 2_7500000)],
                lot: map![&e, (underlying_0, 00_0000001)],
                block: 50,
            };
            fill_user_liq_auction(&e, &auction_data, &samwise, &frodo);
