...
use crate::{errors::TokenError, public_types::Metadata, storage_types::DataKey};
...
pub fn read_name(e: &Env) -> Result<Bytes, TokenError> {
    match read_metadata(e) {
        Metadata::Token(token) => Ok(token.name),
    }
}

pub fn read_symbol(e: &Env) -> Result<Bytes, TokenError> {
    match read_metadata(e) {
        Metadata::Token(token) => Ok(token.symbol),
    }
}

pub fn read_decimal(e: &Env) -> Result<u32, TokenError> {
    match read_metadata(e) {
